...
export default function SecondaryThumbnail({isFigured, figure, text, icone} : propInterface) {
    return(
        <div className="secondaryThumbnail secondaryThumbnailStyle">
            {isFigured && <Figure number={figure}/>}
            <p>{text}</p>
            <Icone srcIcone={icone}/>
            <NextButton styleClassName="btnStyle5" mainClassName="tasksButton" text="voir les tâches" />
        </div>
    )
}